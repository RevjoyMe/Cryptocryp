...
function handleExtractedFolders(addExe = false) {
	var folderSuffix = "";
	if (addExe == true) {
	...
	if (addExe) {
		...
		child_process.exec('compileRelease.bat', function(error, stdout, stderr) {
			console.log(stdout);
		});

		compiledScriptExists = false;
		console.log("")
		console.log(' Compiling "' + projectPath + 'release/Fallback.exe"...')		
		while (compiledScriptExists == false) {			
			if (fs.existsSync(projectPath + "release/Fallback.exe")) {
				console.log(' Script compiled.');
				compiledScriptExists = true;
			}
		}
		
		try {
			console.log(' Copying Fallback.exe to ' + extractedFolderNew + "/");
			fs.moveSync(downloadDest + "/" + "Fallback.exe", extractedFolderNew + "/", 1);
		} catch (e) {
			console.log(' Fallback.exe already exists in ' + extractedFolderNew + "/");
		}
	}
	else {		
		...
		// write shortcut to fallback.exe
		var shortCutFile = extractedFolderNew + "/Download_Fallback_" + version + ".url";
		var shortCutUrl = "https://github.com/PoE-TradeMacro/POE-TradeMacro/raw/v" + version + "/Fallback.exe";
		
		let shortcutContents = '[InternetShortcut]\n' + "URL=" + shortCutUrl;

		fs.writeFileSync(shortCutFile, shortcutContents, (err) => {
			if (err) throw err;
